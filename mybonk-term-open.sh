...
usage() {
  ...
  echo "ssh -o TCPKeepAlive=no -t username@hostname \"cd '$remote_directory' ; tmuxinator start console -n '$hostname' -p .tmuxinator.yml -c node='$hostname' ; sh --login\""
  echo "Options:"
...
command="cd '$remote_directory' && tmuxinator start console -p .tmuxinator.yml -n "${session_name}" -c node='$hostname' && sh --login"
echo $command
